...
from config import TUP_LISTESI, SU_LISTESI, ODEME_TIPLERI

class PesinSatisDialog(tk.Toplevel):
    def __init__(self, parent, app):
        super().__init__(parent)
        self.title("Peşin Satış")
        self.geometry("520x520")
        self.resizable(False, False)
        self.app = app
        self.db = app.db
        self.pesin_service = app.pesin_satis_service
    ...
    def _setup_ui(self):
        ttk.Label(self, text="🛒 Peşin Satış İşlemi",
                 font=("Arial", 14, "bold"), foreground="#8B0000").pack(pady=(10, 5))

        form = ttk.Frame(self)
        form.pack(pady=10, padx=20, fill=tk.X)

        ttk.Label(form, text="Müşteri Adı:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.ent_musteri = ttk.Entry(form, width=30)
        self.ent_musteri.insert(0, "Yürüyen Müşteri")
        self.ent_musteri.grid(row=0, column=1, padx=5, pady=5)

        ttk.Separator(form, orient=tk.HORIZONTAL).grid(
            row=1, column=0, columnspan=3, sticky=tk.EW, pady=5)

        ttk.Label(form, text="Ürün Kategorisi:").grid(row=2, column=0, sticky=tk.W, pady=5)
        kat_frame = ttk.Frame(form)
        kat_frame.grid(row=2, column=1, sticky=tk.W, padx=5)
        ttk.Radiobutton(kat_frame, text="Tüpler", variable=self.kat_secim,
                        value="tup", command=self._kategori_degisti).pack(side=tk.LEFT, padx=2)
        ttk.Radiobutton(kat_frame, text="Sular", variable=self.kat_secim,
                        value="su", command=self._kategori_degisti).pack(side=tk.LEFT, padx=2)

        ttk.Label(form, text="Ürün:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.cmb_urun = ttk.Combobox(form, width=30, state="readonly")
        self.cmb_urun.grid(row=3, column=1, padx=5, pady=5)
        self.cmb_urun.bind("<<ComboboxSelected>>", self._urun_secildi)

        ttk.Label(form, text="Adet:").grid(row=4, column=0, sticky=tk.W, pady=5)
        self.ent_adet = ttk.Entry(form, width=10)
        self.ent_adet.insert(0, "1")
        self.ent_adet.grid(row=4, column=1, sticky=tk.W, padx=5, pady=5)
        self.ent_adet.bind("<KeyRelease>", self._hesapla)

        ttk.Label(form, text="Birim Fiyat:").grid(row=5, column=0, sticky=tk.W, pady=5)
        self.ent_fiyat = ttk.Entry(form, width=12)
        self.ent_fiyat.grid(row=5, column=1, sticky=tk.W, padx=5, pady=5)
        self.ent_fiyat.bind("<KeyRelease>", self._hesapla)

        ttk.Label(form, text="Ödeme Türü:").grid(row=6, column=0, sticky=tk.W, pady=5)
        self.cmb_odeme = ttk.Combobox(form, values=ODEME_TIPLERI, width=12, state="readonly")
        self.cmb_odeme.set("Nakit")
        self.cmb_odeme.grid(row=6, column=1, sticky=tk.W, padx=5, pady=5)

        ttk.Label(form, text="Açıklama:").grid(row=7, column=0, sticky=tk.W, pady=5)
        self.ent_aciklama = ttk.Entry(form, width=30)
        self.ent_aciklama.grid(row=7, column=1, padx=5, pady=5)

        ttk.Label(form, text="Tarih:").grid(row=8, column=0, sticky=tk.W, pady=5)
        self.ent_tarih = ttk.Entry(form, width=15)
        self.ent_tarih.insert(0, datetime.now().strftime("%d.%m.%Y"))
        self.ent_tarih.grid(row=8, column=1, sticky=tk.W, padx=5, pady=5)

        self.lbl_toplam = ttk.Label(self, text="Toplam: 0,00 TL",
                                     font=("Arial", 16, "bold"), foreground="#006600")
        ...
        ttk.Button(btn_frame, text="Peşin Satış Yap →", command=self._kaydet,
    ...
    def _urun_secildi(self, event=None):
        ...
        if fiyat > 0:
            ...
            self.lbl_stok.config(text=f"⚠ {urun} fiyatı girilmemiş! Manuel giriniz.",
                                foreground="orange")
        self._stok_kontrol()
        self._hesapla()
    ...
    def _kaydet(self):
        try:
            ...
            self.pesin_service.pesin_satis_yap(
                tarih=tarih, musteri_adi=musteri, urun_adi=urun,
                adet=adet, birim_fiyat=fiyat, odeme_turu=odeme, aciklama=aciklama)

            toplam = adet * fiyat
            self.app.right_panel.son_islem_ekle(
                f"Peşin Satış: {musteri} - {toplam:,.2f} TL ({adet}x{urun})")

            messagebox.showinfo("Başarılı",
                f"✅ Peşin satış tamamlandı!\n\nMüşteri: {musteri}\n"
                f"Ürün: {adet} x {urun}\nToplam: {toplam:,.2f} TL\nÖdeme: {odeme}")
            self.destroy()
